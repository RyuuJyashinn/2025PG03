...
int main() {
    ...
    for_each(emails.begin(), emails.end(), [&](const string& email) {
        size_t g_pos = email.find('g'); 
        string the_num = email.substr(g_pos + 1, 4);
        studentIDs.push_back(the_num);
        });

    vector<int> indices(emails.size());
    ...
    for (int i = 0; i < indices.size(); ++i) {
        for (int j = i + 1; j < indices.size(); ++j) {
            if (studentIDs[indices[i]] > studentIDs[indices[j]]) {
    ...
    for (int i = 0; i < indices.size(); ++i) {
        int index = indices[i];
        emails_output.push_back(emails[index]);
    }

    for_each(emails_output.begin(), emails_output.end(), [](const string& email) {
        cout << email << endl;
        });
